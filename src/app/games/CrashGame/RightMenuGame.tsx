...
const RightMenuGame = () => {
  ...
  const disableInputs = isAutoBet || isBetting || isPreBet;
  ...
  const onAutoBetChange = e => {
    ...
    if (checked) {
      const hideConfirm = localStorage.getItem(`${HIDE_AUTOBET_LS_NAME}-crash`);
      if (hideConfirm) {
        enableAutoBet();
      } else {
  ...
  const clickSetToMax = () => {
    setAutoCashOut(maxAutoCashout);
  };

  const validAutoCashout = e => {
    if (e.target.value > maxAutoCashout) {
      setAutoCashOut(maxAutoCashout);
    } else {
      ...
      <Collapse in={expandMobile || !isMobile}>
        ...
        <CommonField
          ...
          className={`autoCashout-input ${
            isAutoBet && !autoCashOut ? 'glow' : ''
          }`}
          label={t(...messages.autoCashOut())}
          name="auto-cashout"
          value={autoCashOut || ''}
          onChange={validAutoCashout}
          disabled={disableInputs}
          rightBtnText="Set To Max"
          onClickRightBtn={clickSetToMax}
        />
